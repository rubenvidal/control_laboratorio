class Prueba < ActiveRecord::Base
  ...
  has_one :control
  attr_accessible :pasta_id, :fecha, :horno_id, :partida, :composiciones_attributes, :control_attributes
  accepts_nested_attributes_for :composiciones, :reject_if => lambda { |a| a[:porcentaje].blank?}, :allow_destroy => true
  accepts_nested_attributes_for :control, :reject_if => lambda { |a| a[:valor_x].blank?}, :allow_destroy => true
